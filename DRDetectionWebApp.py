...
st.markdown("""
            Then we use the trained CNN model to detect some signs of Diabetic Retinopathy and show you the result.
            """)
...
if input_image:
    ...
    datagen = ImageDataGenerator(rescale=1.0 / 255.0, preprocessing_function=load_ben_color)
    testdata = datagen.flow(img_rgb_resized)

    if st.button('Click for checking Diabetic Retinopathy'):
        with st.spinner('Predicting...'):
            time.sleep(2)

        predict = model.predict(testdata)
        probability = predict[0] * 100
        Classes = np.argmax(predict)
        class_names = ["No DR", "DR"]  # Replace with your class names
